...
func TestNewDefault(t *testing.T) {
	cfg := NewDefault()
	if defaultCache == nil || !reflect.DeepEqual(cfg.certCache, defaultCache) {
		t.Fatalf("Expected default cache to be created and used")
	}
}
...
func TestNew(t *testing.T) {
	...
	if !reflect.DeepEqual(newCfg.certCache, cache) {
		t.Fatalf("Expected certCache to be set to passed in cache, got %v", newCfg.certCache)
	}
}
...
func TestNewWithCache(t *testing.T) {
	...
	cfgWithDefault := Config{
		DefaultServerName: "webscale",
	}

	newCfg = newWithCache(cache, cfgWithDefault)
	if newCfg.DefaultServerName != "webscale" {
		t.Fatalf("Expected server name to be preserved")
	}
}
